 ...
 *
 */
public class PD1DConfigFileBuilder {
    ...
    private TextField[] replicates;

    private ComboBox[][] abundanceValue;
    private HashMap<String,Integer> index;

    public PD1DConfigFileBuilder(Stage parentStage) {
            ...
            if(currentStep.isEqualTo(0).get()) {
                ...
                for (int j=0; j<temperatures.length; j++) {
                    for(int i=0; i<replicates.length; i++)  {
                        configFileString += abundanceValue[i][j].getValue() + "\t" + temperatures[j].getText() + "\t" + replicates[i].getText() + "\n";
                    }
                }
                try {
    ...
    private Parent configBuilderStep2() {
        ...
        GridPane step2grid = new GridPane();
            ...
            replicates = new TextField[4];

            step2grid.add(new Label("Temp \\ Rep"), 0, 0);

            replicates[0] = new TextField();
            replicates[0].setPrefWidth(150);
            replicates[0].setText("V1");
            step2grid.add(replicates[0], 1, 0);

            replicates[1] = new TextField();
            replicates[1].setPrefWidth(150);
            replicates[1].setText("V2");
            step2grid.add(replicates[1], 2, 0);

            replicates[2] = new TextField();
            replicates[2].setPrefWidth(150);
            replicates[2].setText("T1");
            step2grid.add(replicates[2], 3, 0);

            replicates[3] = new TextField();
            replicates[3].setPrefWidth(150);
            replicates[3].setText("T2");
            step2grid.add(replicates[3], 4, 0);
            ...
            abundanceValue = new ComboBox[replicates.length][temperatures.length];
            ...
            try {
                ...
                if(counter > headers.size()) {
                  alert.setContentText("Warning: " + headers.size() + " number of headers extracted exceeds " + replicates.length * temperatures.length + " (number of temp points x 4)");
                }
                else {
